...
"""
import asyncio, json, signal, logging
...
async def main() -> None:
    # ---- Görünür başlangıç ve config yükleme ----
    ...
    await client.bootstrap_exchange(cfg)  # margin_mode (ISOLATED/CROSS) + position_mode (ONE_WAY)
    ...
    strategy: StrategyBase = strat_cls(cfg["strategy"])

    # ---- Supervisor / Emir katmanı / Reconciler ----
    ...
    for s in (signal.SIGINT, signal.SIGTERM):
        try:
            ...
            pass  # bazı ortamlarda (ör. Windows) sinyal handler yok

    # ---- İç döngüler ----
    async def strat_loop():
        async for event in stream.events():  # {"type":"bar_closed", "symbol": "...", ...}
    ...
    async def trailing_loop():
        interval = cfg["trailing"].get("update_interval_sec", 30)
        while not stop.is_set():
            try:
                await router.update_trailing_for_open_positions(stream, cfg["trailing"])
            except Exception as e:
